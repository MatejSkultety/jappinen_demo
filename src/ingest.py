...
def ingest_excel_file(excel_path: str | Path) -> list[str]:
    excel_path = Path(excel_path)
    frames = pd.read_excel(excel_path, sheet_name=None)
    prefix = f"{normalize_name(excel_path.stem)}__"
    ...
    with get_connection() as connection:
        ...
        for sheet_name, frame in frames.items():
            table_name = table_name_for_sheet(excel_path.name, sheet_name)
...
def tables_for_file(file_name: str) -> list[str]:
    prefix = f"{normalize_name(Path(file_name).stem)}__"
    with get_connection() as connection:
